...
:root{--ground:#f4f4f6;--cosmic:#ffffff;--raised:#ebebee;--line:#dedee2;--hover-strong:#c9c9cf;--fg:#17171a;
  --dim:#56565e;--accent:#b0580f;--accent-ink:#ffffff;--paper:#ffffff;--paper-line:#dedee2}
@media (prefers-color-scheme:dark){:root:not([data-theme="light"]){--ground:#141416;--cosmic:#1c1c1f;
  --raised:#242427;--line:#2e2e33;--hover-strong:#4a4a52;--fg:#ececef;--dim:#a2a2a9;--accent:#ffa24c;
...
:root[data-theme="dark"]{--ground:#141416;--cosmic:#1c1c1f;--raised:#242427;--line:#2e2e33;--hover-strong:#4a4a52;
  --fg:#ececef;--dim:#a2a2a9;--accent:#ffa24c;--accent-ink:#141416;--paper-line:#2e2e33}
...
.pick input:checked + .tile::after{border-color:#b0580f;background-color:#b0580f;background-image:url("__CHECK__")}
...
def logo_inner(name):
    ...
    return view[:view.index('"')], text[text.index(">") + 1:text.rindex("</svg>")]
...
def final_section():
    ...
    symbols = "".join(f'<symbol id="final-{key}" viewBox="{logo_inner(name)[0]}">{logo_inner(name)[1]}</symbol>'
                      for key, name in (("mark", "mark.svg"), ("small", "mark-small.svg"), ("circle", "mark-circle.svg")))
    ...
    lockups = "".join(
        f'<div class="build-tile {theme}"><svg viewBox="{view}" class="lockup" role="img" aria-label="Echo Studio">{body}</svg></div>'
    ...
    icons = "".join(f'<span class="size"><img src="{png(name)}" width="{px}" height="{px}" alt="">{px}</span>'
                    for name, px in (("128x128.png", 128), ("64x64.png", 64), ("Square44x44Logo.png", 44),
    ...
    circle = "".join(f'<div class="build-tile {theme}">{use("circle", cls="big")}<span class="sizes">'
                     + "".join(f'<span class="size">{use("circle", px)}{px}</span>' for px in (48, 32, 24)) + "</span></div>"
                     for theme in ("light", "dark"))
    scenes = "".join(
        f'<div class="scene {theme}"><div class="titlebar">{use("small", 16)}<span>Echo Studio</span>'
    ...
    return (f'<section class="wave final"><div class="wave-head"><h2>Готовый знак</h2>'
            f'<p class="prompt">2C — точка и отклик, дальний рассыпается на три штриха. Янтарь #ffa24c без подложки '
            f'в иконке и шапке; янтарный круг с глифом 72 % — запасной вариант для аватаров. Геометрия — '
            f'brand/logo/generate.py, иконки — brand/make-icons.py: до 32 px берётся упрощённый знак с толстой '
            f'линией и широкими просветами.</p></div>'
            ...
            f'<div class="build"><h3>В круге</h3><div class="build-row">{circle}</div></div>'
...
def main():
    ...
    html = (f'<title>Знак Echo Studio</title><style>{fonts}{CSS}{"".join(getattr(m, "CSS", "") for m in mods)}</style>'
            f'<main class="wrap"><header><h1>Знак Echo Studio</h1>'
            f'<p class="brief">Иконка приложения, панель задач, шапка окна и About. Смысл: эхо, студия инструментов Claude, сессии. '
