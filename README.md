...
- Inline markdown rendering with syntax highlighting; links open in your default browser
- Edit head fields and body — edits go to `index.db` (FTS index kept in sync); vault markdown is regenerated by the next `memory reindex`
- Archive (with reason) / restore / hard-delete
- Bulk select and bulk action bar
- **Backup before every write** — last 20 copies of `index.db` kept under `~/.memory/.backups/`
...
- Custom and AI-generated session titles (with `/rename` support in supported agent CLIs, falls back to UUID prefix when neither is set); renames made in the app are stored in `~/.claude/echo-studio-titles.json`
...
- Keyboard: `Esc` exits the session viewer (first `Esc` leaves the filter field)
...
- The only network calls are to your Ollama instance when you use semantic search — `embedding.base_url` from `~/.memory/config.yaml`, default `http://localhost:11434`. Nothing else.
- Strict content security policy; file operations from the UI are limited to `~/.claude/projects` (sessions) and `.md` exports.
- Memory writes only happen when you confirm an action — and a backup is taken first.
